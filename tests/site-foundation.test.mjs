...
test('LocalMind 상세 페이지는 비개발자 설명과 접근 가능한 시각화를 제공한다', () => {
  ...
  assert.match(project, /class="architecture-flow architecture-flow-compact"/);
  assert.match(detailPage, /@media \(max-width: 38rem\)[\s\S]+architecture-flow-compact/);
  ...
  assert.match(project, /class="architecture-flow architecture-flow-compact" role="img"/);
